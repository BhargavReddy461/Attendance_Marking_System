	...
	<div class="container bg" style="margin-bottom:100px">
		...
		<table class="table table-hover" style="font-size:24px">
			<tr class="table-info">
				...
				<th class="table-info">approval status</th>
				<th class="table-info">Delete</th>
			</tr>
			<c:forEach items="${sessionsenrolled}" var="l">
				<tr>
					<td>${l.userId}</td>
					<td>${l.sessionId}</td>
					<td>${l.sessionDesc}</td>
					<td><a href="approveenrollment?id=${l.id}">${l.approvalStatus}</td>
					<td><a href="rejectenrollmentrequest?id=${l.id}">reject</td>
				</tr>
			</c:forEach>
