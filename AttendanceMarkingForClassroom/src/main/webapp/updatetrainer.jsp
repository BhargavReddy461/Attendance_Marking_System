...
<body background="${contextPath}/resources/img/intro-bg.jpg">
	...
	<div class="container">
        ...
        <form:form method="POST" modelAttribute="trainer1"  action="/updatetrainerdetails" class="form-signin">
            <h2 class="form-signin-heading">Update Trainer Details</h2>
            <spring:bind path="trainerId">
                <div class="form-group ${status.error ? 'has-error' : ''}">
                    <form:input type="text" path="trainerId" class="form-control" value="${trainer1.trainerId}"  placeholder="Trainer Id"
                                autofocus="true" readOnly="true"></form:input>
                    <form:errors path="trainerId"></form:errors>
                </div>
            </spring:bind>
            <spring:bind path="trainerName">
                <div class="form-group ${status.error ? 'has-error' : ''}">
                    <form:input type="text" path="trainerName" class="form-control"   placeholder="Trainer Name"
                                autofocus="true" ></form:input>
                    <form:errors path="trainerName"></form:errors>
                </div>
            </spring:bind>
            <spring:bind path="contactNo">
                <div class="form-group ${status.error ? 'has-error' : ''}">
                    <form:input type="text" path="contactNo" class="form-control" placeholder="Contact No"
                                autofocus="true"></form:input>
                    <form:errors path="contactNo"></form:errors>
                </div>
            </spring:bind>
            <spring:bind path="traineremail">
                <div class="form-group ${status.error ? 'has-error' : ''}">
                    <form:input type="text" path="traineremail" class="form-control" placeholder="Email"
                                autofocus="true"></form:input>
                    <form:errors path="traineremail"></form:errors>
                </div>
            </spring:bind>
